...
# Part to specify which volume I want to parse with Python
volume = 2
...
# Create DataFrame with all files + details
...
# One specific list of items matching set volume
Dune = df[df["volume"] == str(volume)]["path"]

# Read all files into one list (each element is a separate file)
content = []
for file in Dune:
    with open(file, mode="r", encoding="UTF-8") as f:
        content.append(f.readlines())

# Real all parts into one big list divided into paragraphs
book = []
for k in range(0, len(content)):
    for i in range(0, len(content[k])):
        book.append(content[k][i])

# Classify each paragraph based on class_dictionary
book_classified = []
